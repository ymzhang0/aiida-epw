...
class EpwParser(BaseParser):
    ...
    def parse(self, **kwargs):
        ...
        parsed_epw, logs = self.parse_stdout(stdout, logs, code_version = Version(parsed_data['code_version']))
        ...
        if EpwCalculation._output_elbands_file in self.retrieved.base.repository.list_object_names():
            ...
            self.out('el_band_structure', self.parse_bands(elbands_contents, self.node.inputs.kfpoints, 'eV'))
        ...
        if EpwCalculation._output_phbands_file in self.retrieved.base.repository.list_object_names():
            ...
            self.out('ph_band_structure', self.parse_bands(phbands_contents, self.node.inputs.qfpoints, 'meV'))
        ...
        if EpwCalculation._OUTPUT_DOS_FILE in self.retrieved.base.repository.list_object_names():
            dos_contents = self.retrieved.base.repository.get_object_content(EpwCalculation._OUTPUT_DOS_FILE)
            self.out('dos', self.parse_dos(dos_contents))

        if EpwCalculation._OUTPUT_PHDOS_FILE in self.retrieved.base.repository.list_object_names():
            phdos_contents = self.retrieved.base.repository.get_object_content(EpwCalculation._OUTPUT_PHDOS_FILE)
            self.out('phdos', self.parse_phdos(phdos_contents))

        if EpwCalculation._OUTPUT_PHDOS_PROJ_FILE in self.retrieved.base.repository.list_object_names():
            phdos_proj_contents = self.retrieved.base.repository.get_object_content(EpwCalculation._OUTPUT_PHDOS_PROJ_FILE)
            self.out('phdos_proj', self.parse_phdos(phdos_proj_contents))

        if EpwCalculation._OUTPUT_A2F_PROJ_FILE in self.retrieved.base.repository.list_object_names():
            a2f_proj_contents = self.retrieved.base.repository.get_object_content(EpwCalculation._OUTPUT_A2F_PROJ_FILE)
            self.out('a2f_proj', self.parse_a2f_proj(a2f_proj_contents))

        if EpwCalculation._OUTPUT_LAMBDA_FS_FILE in self.retrieved.base.repository.list_object_names():
            lambda_FS_contents = self.retrieved.base.repository.get_object_content(EpwCalculation._OUTPUT_LAMBDA_FS_FILE)
            self.out('lambda_FS', self.parse_lambda_FS(lambda_FS_contents))

        if EpwCalculation._OUTPUT_LAMBDA_K_PAIRS_FILE in self.retrieved.base.repository.list_object_names():
            lambda_k_pairs_contents = self.retrieved.base.repository.get_object_content(EpwCalculation._OUTPUT_LAMBDA_K_PAIRS_FILE)
        ...
        if imag_iso_filecontents != []:
            ...
            for T, imag_iso_filecontent in imag_iso_filecontents:
                iso_gap_function = self.parse_gap_function(imag_iso_filecontent, skiprows=1)
                iso_gap_functions_arraydata.set_array(T.replace('.', '_'), iso_gap_function)
